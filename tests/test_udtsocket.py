...
class UDTServer(udt.udpserver.UDPServer):
	...
	def handle_packet(self, client):
		header_size = udt.udtsocket.ControlHeader._struct.size
		handshake_size = udt.udtsocket.HandshakePacket._struct.size
		...
		while 1:
			...
			if h.get_msg_type() == udt.udtsocket.handshake:

				hd_buff = client.get_bytes(handshake_size)
				if hd_buff:
					bufferio = udt.udtsocket.BytesIO(header_size + handshake_size)
					bufferio.write(h_buff)
					bufferio.write(hd_buff)
